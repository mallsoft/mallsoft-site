export const me = {
  email: 'box@melaa.net',
  name: 'Hans',
  github: '/mallsoft',
  repoUrl: 'https://github.com/mallsoft/mallsoft-site',
  keywords: [
    'Node',
    'Express',
    'SvelteKit',
    'Javascript',
    'Polka',
    'CSS',
    'HTML5',
    'Typescript',
    'ARIA',
    'Postgres',
    'Sass',
    'Svelte',
    'GPT3',
    'React',
    'rollup',
    'Coffee',
    'Heroku',
    'Sapper',
    'Relaxing',
    'Humor',
    'figma',
    'Squidex',
    'Sentry',
    'Internet',
    'Bouldering',
    'graphql',
    'firebase',
    'Affinity Designer',
    'REST',
    'CI/CD',
    'Affinity Photo',
    'Adobe XD',
    'Adobe Illustrator',
    'Adobe Photoshop',
    'Docker',
    'Python',
    'Vscode',
    'Git',
    'Azure',
    'Copilot',
    'SQL'
  ]
};

export const projectlog = [
  {
    title: 'Wove',
    description: 'Landing page for Wove marketing agency',
    link: 'https://www.wove.no/',
    image: 'projectimage/Wove.webp',
    alt: 'wove.no faximile'
  },
  {
    title: 'Nicode',
    description: 'Landing page for nicode consultancy',
    link: 'https://nicode.dev/',
    image: 'projectimage/Nicode.webp',
    alt: 'lysaskroken.no faximile'
  },
  {
    title: 'Lyll',
    description: 'Landing page for the lyll advertisement platform',
    link: 'https://lyll.io/',
    image: 'projectimage/lyll.webp',
    alt: 'lyll.io faximile'
  },
  {
    title: 'Nygardsportalen',
    description: 'Landing page and estateselector',
    link: 'https://www.nygardsportalen.no/',
    image: 'projectimage/Nygardsportalen.webp',
    alt: 'nygardsportalen.no faximile'
  },
  {
    title: 'Lysaskroken',
    description: 'Landing page and estateselector',
    link: 'https://www.lysaskroken.no/',
    image: 'projectimage/Lysaskroken.webp',
    alt: 'lysaskroken.no faximile'
  },
  {
    title: 'Smestadaasen',
    description: 'Landing page and estateselector',
    link: 'https://smestadaasen.no/',
    image: 'projectimage/Smestadaasen.webp',
    alt: 'smestadaasen.no faximile'
  },
  {
    title: 'Åstveitvågen Park',
    description: 'Landing page and estateselector',
    link: 'https://www.astveitvagenpark.no/',
    image: 'projectimage/Aastveitvagenpark.webp',
    alt: 'astveitvagenpark.no faximile'
  },
  {
    title: 'Thriving Ventures',
    description: 'Landing page saas startup in the gaming industry',
    link: 'https://thrivingventures.com/',
    image: 'projectimage/Thrivingventures.webp',
    alt: 'thrivingventures.com faximile'
  },
  {
    title: 'Melaa.net',
    description: 'Early website for me in vanilla js, html and css',
    link: 'https://melaa.net/',
    image: 'projectimage/melaanet.webp',
    alt: 'melaa.net faximile'
  }
];
